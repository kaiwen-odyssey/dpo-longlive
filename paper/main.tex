...
\midrule
Training (paired)    & 1000 & VQ 987, MQ 997, TA 986 \\
...
\caption{Chunk-wise teacher-forcing DPO training dynamics, one curve per reward head (MQ blue, TA green, VQ red). \textbf{Trained on a paired pool of 1000 VidProm prompts}, batch size 1, $\beta=500$, learning rate $2\mathrm{e}{-}5$, 30 gradient steps. The implicit margin is the inside term $-\beta/2 \cdot \big((\|f_\theta(x_t^w)-\tau^w\|^2-\|f_{\theta_0}(x_t^w)-\tau^w\|^2)-(\|f_\theta(x_t^l)-\tau^l\|^2-\|f_{\theta_0}(x_t^l)-\tau^l\|^2)\big)$; values above zero mean the policy assigns higher implicit log-probability to the chosen video than the reference does.}
\label{fig:dpo}
\end{figure}
...
\caption{Offline reward-DMD training dynamics. \textbf{Trained on the same 1000-prompt paired pool} as Figure~\ref{fig:dpo} (now interpreted as $2{\times}1000=2000$ rollouts since each preference pair contributes both videos), batch size 1, $\beta=2.0$, learning rate $2\mathrm{e}{-}5$, 30 gradient steps. Per-step reward weight $\exp(\beta \cdot r)$ and the gap between the policy's and the reference's denoising loss; the latter is the chunk-wise analogue of the DMD score difference used in Reward-Forcing~\cite{lu2025rewardforcing}.}
\label{fig:redmd}
\end{figure}
...
\midrule
DPO --- target MQ & $-0.084$ (-0.021) & $+0.282$ (+0.195) & $+0.863$ (-0.037) & $+1.061$ (+0.137) \\
...
\textbf{v2} & \textbf{500} & \textbf{2e-5} & \textbf{30} & \textbf{1} & \textbf{0} & \textbf{0} & \textbf{$+0.195$} & \textbf{$+0.137$} & \textbf{0.60} & \textbf{0.65} \\
v3 & 500 & 2e-5 & 400 & 4 & 1.0 & 0.20 & $-0.003$ & $-0.034$ & 0.55 & 0.45 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Train vs.\ held-out DPO metrics for the v3 dpo\_MQ recipe ($\beta=500$, lr$=2\mathrm{e}{-}5$, grad\_accum$=4$, anchor $\alpha=1.0$, $\delta_{\rm gap}=0.2$, 400 steps). Train: final 20-step window. Eval: same teacher-forced DPO forward on $n=20$ pairs built from two fresh base rollouts (seeds $1000$ and $1007$, matching the training distribution) on each held-out prompt, labelled chosen/rejected by MQ (no ties to drop), $K=4$ random $t$-grid samples averaged per pair. Held-out MQ gap is $+0.303\pm0.224$, comparable to the $\delta_{\rm gap}=0.2$ filtered training pool, so the pairs are equally easy. Accuracy drops by $-0.14$ to below chance and the implicit margin flips sign by approximately one full unit --- the textbook over-fitting signature, with only the prompt distribution differing between train and eval.}
\label{tab:overfit}
\begin{tabular}{lrr}
\toprule
 & Train (final 20 steps) & Eval ($n=20$ held-out pairs) \\
\midrule
DPO accuracy        & $0.62$              & $\mathbf{0.48 \pm 0.25}$ \\
Implicit margin     & $+0.438$         & $\mathbf{-0.554 \pm 2.28}$ \\
DPO loss            & $1.316$            & $1.889 \pm 1.89$ \\
Win-rate (margin$>0$)& $0.71$\,($=58/82$) & $0.45$\,($=9/20$) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{v4 dpo\_MQ trajectory: held-out (eval-50) and training-pool (train-window-100) mean MQ and win-rate at every 100-step checkpoint. Configuration: $\beta=500$, lr$=5\mathrm{e}{-}6$, anchor $\alpha=0.5$, batch 1, no $\delta_{\rm gap}$ filter, trained on the full unfiltered $997$-pair MQ pool for $1000$ optimizer steps on a single Blackwell GPU ($9.4$\,h wall time). Eval-50 = $20$ held-out VidProm prompts (from the same pool used by Tables~\ref{tab:main} and~\ref{tab:overfit}) plus $30$ fresh VidProm prompts sampled with no overlap to train, $50$ total. Each policy rollout uses the matched per-prompt seed $1000 + 31 \cdot \text{pid}$ so the noise tensor is identical to the cached base rollout at the same prompt — the per-prompt comparison only varies the model weights. Win-rate counts the number of prompts where the policy's MQ score beats its cached base. Held-out reward baseline (LongLive base on eval-50): mean MQ $+0.0565$. The bolded step~$900$ row is the peak; \texttt{policy\_best.pt} stores those weights.}
\label{tab:v4traj}
\begin{tabular}{rrrrr}
\toprule
step & eval-50 $\Delta$\,MQ & eval-50 win & train-100 mean MQ & train-100 win \\
\midrule
100 & $-0.0624$ & 0.40 & $+0.046$ & 0.52 \\
200 & $-0.0685$ & 0.46 & $+0.299$ & 0.48 \\
300 & $-0.0732$ & 0.36 & $+0.079$ & 0.47 \\
400 & $-0.0053$ & 0.54 & $+0.247$ & 0.61 \\
500 & $-0.0102$ & 0.46 & $+0.113$ & 0.56 \\
600 & $+0.0132$ & 0.52 & $+0.107$ & 0.53 \\
700 & $+0.0850$ & 0.50 & $+0.469$ & 0.61 \\
800 & $+0.0713$ & 0.60 & $+0.140$ & 0.51 \\
\textbf{900} & \textbf{$+0.1043$} & \textbf{0.70} & \textbf{$+0.069$} & \textbf{0.61} \\
1000 & $+0.0667$ & 0.56 & $+0.196$ & 0.58 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{v7 dpo\_MQ aggregated trajectory: held-out (eval-50) and training-pool (train-window-100) mean MQ and win-rate at every 100-step checkpoint. Configuration: $\beta=100$, lr$=2\mathrm{e}{-}5$, anchor $\alpha=0.5$, aggregated chunk-wise DPO loss (single sigmoid on rollout-mean of per-block margins, shared $(t,\epsilon)$ across all 7 chunks), batch 1, no $\delta_{\rm gap}$ filter, trained on the full unfiltered $997$-pair MQ pool for $1000$ optimizer steps on a single Blackwell GPU. Eval-50 = same $50$ held-out VidProm prompts as Table~\ref{tab:v4traj}; each policy rollout uses matched per-prompt seed $1000 + 31\cdot\text{pid}$ so noise is identical to cached base. Held-out reward baseline: mean MQ $+0.0565$. The bolded row is the peak by eval-50 mean MQ (saved as \texttt{policy\_best.pt}). Note the bimodal trajectory: half of checkpoints land in the strongly-positive basin ($\Delta\ge+0.40$) and two checkpoints (200, 600) collapse to $\Delta\approx-0.5$ — a real instability of aggregated mode at this lr but the policy recovers within 100 steps each time.}
\label{tab:v7traj}
\begin{tabular}{rrrrr}
\toprule
step & eval-50 $\Delta$\,MQ & eval-50 win & train-100 mean MQ & train-100 win \\
\midrule
100 & $+0.0990$ & 0.56 & $+0.230$ & 0.63 \\
200 & $-0.5097$ & 0.30 & $-0.253$ & 0.16 \\
300 & $+0.5360$ & 0.86 & $+0.569$ & 0.81 \\
\textbf{400} & \textbf{$+0.8016$} & \textbf{0.86} & \textbf{$+1.010$} & \textbf{0.89} \\
500 & $+0.5688$ & 0.82 & $+0.691$ & 0.77 \\
600 & $-0.4506$ & 0.28 & $-0.315$ & 0.27 \\
700 & $+0.4036$ & 0.72 & $+0.633$ & 0.64 \\
800 & $+0.4035$ & 0.76 & $+0.560$ & 0.75 \\
900 & $+0.4287$ & 0.74 & $+0.389$ & 0.69 \\
1000 & $+0.4266$ & 0.70 & $+0.474$ & 0.75 \\
\bottomrule
\end{tabular}
\end{table}
